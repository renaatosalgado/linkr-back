...
export async function getTwoNamesThatLiked(req, res) {
  ...
  try {
    const { rows: checkLikeRows } = await likeRepositoy.checkLike(userLocals?.id, postId);
    ...
    const { rows: likeRows } = await likeRepositoy.getTwoNamesThatLiked(userLocals?.id, postId);
